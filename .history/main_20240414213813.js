...
let dadosTimeAtual = 1 // mudar para o time 2 e 3
...
function cadastroTime() {
    ...
    let teamName
        ...
        if (teamNameElement && teamNameElement.value.trim() !== '') {
            teamName = teamNameElement.value;
        } else {
            teamName = 'Time A';
        }
    ...
    resetDadosTime(dadosTimeAtual) 

    dadosTimeAtual += 1 
}

function resetDadosTime(timeAtual) {

    if (timeAtual==2) {

        document.querySelector("body > section.info-times > div > div.name > label").textContent = "Nome do time 2"
