...
function resetDadosTime() {
    ...
    // Check if dadosTimes[timeAtual] is defined before accessing properties
    if (dadosTimes[timeAtual]) {
      document.querySelector("body > section.info-times > div > div.name > label").textContent = "Nome do " + dadosTimes[timeAtual].timeLabel;
      // ... rest of the function
    } else {
      console.error("Time data for current index is undefined!");
      // Handle the case where there's no data for the current index (optional)
    }
  }
